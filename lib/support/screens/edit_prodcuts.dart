...
class _AddProductsState extends State<EditProducts> {
  ...
  Future getProductSpecs() async {
    return await FirebaseDatabase.instance
      ...
      sizeController.text = map[AppData.productSpicesAvailable];
      colorController.text = map[AppData.productRestaurantsAvailable];
      ...
      images = map[AppData.productImgURL]
          .toString()
          .replaceAll("[", "")
          .replaceAll("]", "");
  ...
  Future addNewProduct() async {
    ...
    productRef.child(widget.productKey).update({
      AppData.productImgURL: filesURL.toString(),
    ...
    for (int s = 0; s < fbImages.length; s++) {
      if (!fbImages[s].toString().contains("https")) {
        StorageReference ref = FirebaseStorage.instance
            ...
            .child(refKey)
            .child(refKey + "$s.jpg");
        StorageUploadTask uploadTask = ref.putFile(new File(fbImages[s]));
        var downloadURL =
