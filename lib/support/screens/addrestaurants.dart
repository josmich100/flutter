...
class _GirliesBoutiqueState extends State<Restaurants> {
  final TextEditingController _textEditingController =
      new TextEditingController();
  bool _canAddRestaurant = false;
  File catergoryImage;
  bool isAdding = false;
  BuildContext context;
  static DatabaseReference dbRef = FirebaseDatabase.instance.reference();
  static final reference = dbRef.child(AppData.restaurantsDB);
  String editText;
                  ...
                  child: new Row(
                      ...
                      children: <Widget>[
                          ...
                          child: new Container(
                            child: new CircleAvatar(
                              backgroundImage: new NetworkImage(
                                  snapshot.value[AppData.restaurantImgURL]),
                            ),
                          ),
                        ),
                        ...
                        new Flexible(
                          fit: FlexFit.tight,
                          child:
                              new Text(snapshot.value[AppData.restaurantName]),
                        ),
                        new Container(
                          child: new Row(
                            children: <Widget>[
                              new IconButton(
                                  ...
                                  onPressed: () {
                                    _editRestaurant(snapshot: snapshot);
                                  }),
                              new Container(
                                height: 30.0,
                                width: 1.0,
                  ...
                  onChanged: (String messageText) {
                    setState(() {
                      if (catergoryImage != null)
                        _canAddRestaurant = messageText.length > 0;
                    });
                  },
                  onSubmitted: _textMessageSubmitted,
                  decoration: new InputDecoration.collapsed(
                    hintText: "Enter New Restaurant",
                  ),
                ),
              ),
              new Container(
  ...
  Future<Null> _textMessageSubmitted(String text) async {
    ...
    _createRestaurant(restaurantName: text);
  }

  Future setRestaurantIMG() async {
  ...
  Future _createRestaurant({String restaurantName}) async {
    ...
    FirebaseDatabase.instance
        ...
        .child(ref)
        .set({
      AppData.restaurantImgURL: downloadUrl.toString(),
      AppData.restaurantName: restaurantName,
      AppData.keyID: ref,
    }).then((_) {
      setState(() {
        isAdding = false;
  ...
  Future _removeRestaurant({String refKey}) async {
    FirebaseStorage.instance
        .ref()
        .child(AppData.productrestaurants)
        .child(refKey)
        .child(refKey + ".jpg")
        .delete();
    reference.child(refKey).remove();

    showInSnackBar('Restaurant has been removed!');
  }

  Future _editRestaurant({DataSnapshot snapshot}) async {
    setState(() {
      _textEditingController.text = snapshot.value[AppData.restaurantName];
    });
  }
}
